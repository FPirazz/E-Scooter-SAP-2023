...
class PixelArtServiceProxy implements PixelArtAsyncAPI {
	...
	private Histogram promHistogram;
	
	public PixelArtServiceProxy() {
		vertx = Vertx.vertx();
	}
	
	public void init(String host, int port, Histogram prometheusHistogram) {
		this.host = host;
		this.port = port;
		...
		this.promHistogram = prometheusHistogram;
		sendLogRequest("[DashBoard] - Init of the PixelArtServiceProxy.");
	}

	public Future<String> createBrush() {
		//prometheus timer to evelaute API response time.
		Timer responseTimer = promHistogram.labelValues("createBrush").startTimer();
		try {
			...
			// Ferma il timer quando la logica di gestione della richiesta è completata
	...
	public Future<JsonArray> getCurrentBrushes() {
		//prometheus timer to evelaute API response time.
		Timer responseTimer = promHistogram.labelValues("getCurrentBrushes").startTimer();
		try
		{
	...
	public Future<JsonObject> getBrushInfo(String brushId) {
		//prometheus timer to evelaute API response time.
		Timer responseTimer = promHistogram.labelValues("getBrushInfo").startTimer();
		try
		{
